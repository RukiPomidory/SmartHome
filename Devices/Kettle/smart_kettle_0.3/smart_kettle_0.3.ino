...
const double beta = 3600.0;     // Табличный коэффициент термистора (корректируется эмпирически)
...
//--------------------------------
const double biasF = 180;   // Смещение. Нужно для вычета веса самого чайника
const double kF = 0.005;    // Коэффициент. Отношение объема к значению с АЦП

const double biasR = 380;   // Для правого датчика
const double kR = 0.007;    // 

const double biasL = 380;   // Для левого датчика
const double kL = 0.007;    // 

const double K = 0.6;       // Общий коэффициент. Его выделение при наличии отдельных коэффициентов удобно при настройке.
//-------------------------------
