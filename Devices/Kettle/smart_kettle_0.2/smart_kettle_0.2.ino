...
//--------------------------------
const double biasF = 100;   // Смещение. Нужно для вычета веса самого чайника
const double kF = 0.01;     // Коэффициент. Отношение объема к значению с АЦП

const double biasR = 250;   // Для правого датчика
const double kR = 0.01;     // 

const double biasL = 250;   // Для левого датчика
...
void sendData(double tempValue, double waterValue)
{
    ...
    String debugData =  "\nF: " + pressureSensorF.getValue() + 
                        "\nL: " + pressureSensorL.getValue() + 
                        "\nR: " + pressureSensorR.getValue() +
                        "\nTEMP: " + temperatureSensor.getValue();
